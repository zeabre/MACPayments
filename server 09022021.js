		...
		var ApptSchema = new Schema(
			{
				...
				service_ICD10: String,
				email_sent: Date,
				consumed: [ConsumableSchema],
				complete: {type: Number, default: 0},
				cash_paid: {type: Number, default: 0},
				mfi_sent: {type: Number, default: 0},
				mfi_sent_date: Date,
				rep_sent: {type: Number, default: 0},
				rep_sent_date: Date,
...
async function WrapSendMail(){
    ...
    app.post('/api/appt', function(req, res, next) {
        ...
        Appt.create({
				...
				practice_tech: req.body.pt_appt_tech,		
            ...
            Appt.find(function(err, appts) {
                ...
                // update a service
    app.put('/api/appt/:appt_id/:appt_status/:appt_cash', function(req, res, next) {
	
		console.log("Updating : " + req.params.appt_id.toString());
	    Appt.findByIdAndUpdate(req.params.appt_id,{
			complete 	: req.params.appt_status,
			cash_paid 	: req.params.appt_cash,
			}
    ...
    app.post('/api/patientdetail', function(req, res, next) {
       ...
       app.get('/api/appt/:uuid', function (req, res, next) {
		...
		app.get('/api/availabletechs', function (req, res, next) {
		  Account.find({
			'tech': true
    ...
    app.get('/api/patient', function(req, res, next) {
        ...
        Account.find(function(err, accounts) {
            ...
            if (err)
                res.send(err)
		    console.log("Returning values: " + JSON.stringify(accounts, null, 2));
...
*/
	...
	////////////			console.log(xml);
	  ...
	  app.get('/api/maincash', function(req, res, next) {
        // use mongoose to get all doctors in the database
        Appt.aggregate([
	  ...
	  app.get('/api/specificdates/:practice/:sYear/:sMonth/:sDay', function(req, res, next) {
        ...
        Appt.aggregate([
						...
						{
							$match: { 	
												"practice_tech": req.params.practice,
												"appt_date": { $gte: momentDateStart , $lte: momentDateEnd}
											} 
						},
	  ...
	  app.get('/api/specifictechs/:tech', function(req, res, next) {
		console.log('Param: ' + req.params.tech);
		console.log('Body: ' + JSON.stringify(req.params, null, 2));
	...
	app.get('/api/main/:sYear/:sMonth/:sDay', function(req, res, next) {
        ...
        Appt.aggregate([
							...
							$project:
								{
										...
										"appt_date" : 1,
										"service_code" : 1,
										"PatientDetails.pt_fname" : 1,
										"PatientDetails.pt_sname" : 1,
										"PatientDetails.pt_medaid_name" : 1,
										"PatientDetails.pt_medaid_num" : 1,
				...
				var inter2 = JSON.stringify({Patients :{"Patient" : patients} } , null, 2);
				var inter = JSON.stringify({"_declaration":{"_attributes":{"version":"1.0","encoding":"utf-8"}},"Batch" : batch, Patients :{"Patient" : patients} } , null, 2);
				var result = convert.json2xml(inter,options);		
				var ciphertext = CryptoJS.AES.encrypt(JSON.stringify(patients), 'gawie').toString();
				
					
				  var c = new FTPClient();

					  c.on('ready', function() {
						//c.put( __dirname + '/uploads/b15e7736d5424f429d885228fcc90983', 'b15e7736d5424f429d885228fcc90983y', function(err) {
						
						c.put('b15e7736d5424f429d885228fcc90983'   , 'b15e7736d5424f429d885228fcc90983y', function(err) {
				...
				Request.post({
					...
					"url": "http://localhost:3200/api/mfi",
					"form":  	{cipher : ciphertext}
					...
					if(error) {
						...
						if (response.statusCode == 200) {
							...
							var bytes  = CryptoJS.AES.decrypt(response.body, 'gawie');
							var body1 = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));

						//	console.log(body1);
							
								Object.keys(body1).forEach(function(k){

									console.log(k + ' - ' + body1[k]._id);
									Appt.findByIdAndUpdate(body1[k]._id,{
											mfi_sent: 1
									}
									, function(err, appt) {
											if (err)
												res.send(err);   
										console.log("Appt MFI sent updated");
															
									});
								
								});
						} else {
							console.log("Appt MFI update failed");
						}
	  ...
	  app.get('/api/medaid', function(req, res, next) {
        // use mongoose to get all doctors in the database
       
				console.log('Getting med aid');
								
				Request.post({
					
					
					"url": "http://api.mfi.co.za/api.php",
					"headers": {
					...
					"json": 
							[{
								...
								{"get": "Medical_aid",
									"data": "ALL"
									// Options "SCHEME", "ALL" or actual scheme
								}]
...
app.post('/api/medaid2', function(req, res, next) {
        // use mongoose to get all doctors in the database
       
				console.log('Getting med aid2');
	...
	app.get('/api/tech', function(req, res, next) {

        // use mongoose to get all techs in the database
        Tech.find(function(err, techs) {

            // if there is an error retrieving, send the error. nothing after res.send(err) will execute
            if (err)
                res.send(err)
		    console.log("Returning values: " + JSON.stringify(techs, null, 2));
		    res.json(techs); // return all techs in JSON format
        });
    }); 
    
    app.post('/api/tech', function(req, res, next) {

        // create a doctor, information comes from AJAX request from Angular
        Tech.create({
				tech_name: req.body.tech_name,
				tech_id: req.body.tech_id,
				tech_pr: req.body.tech_pr,
				tech_email: req.body.tech_email,
				tech_telnum: req.body.tech_telnum,
				tech_location: req.body.tech_location,
        }, function(err, techs) {
            if (err)
                res.send(err);
			console.log("Creating: " + JSON.stringify(req.body, null, 2));
            // get and return all the techs after you create another
            Tech.find(function(err, techs) {
                if (err)
                    res.send(err)
                res.json(techs);
 ...
 app.post('/api/uploadpatientdetails',  upload.any('files'), function (req, res, next) {
		...
		Upload.insertMany(req.files, function (err, result) {
			  ...
			  if (err) 	
			  {
					...
					return res.status(500).json({status: 'Upload fail'});
				}
			  else
			  {
					...
					//	console.log(id);
						        ...
						        Upload.findByIdAndUpdate(id,{
									appt_id : req.body.appt_id
									}
								, function(err, uploads) {
									if (err)
										return res.status(500).json({status: 'Upload update fail'});
					...
					return res.status(200).json({status: 'Upload successful: ' + result});
