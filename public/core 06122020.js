  ...
  .config(['$routeProvider',
   ...
   angular.module('scotchApp')
	.controller('MainController',['$scope','$http', function($scope, $http) {
			...
			checkBookings = function(m) {
							console.log('In here ' + m);
							if(m == 0)
							{
...
//							if (clearing) {
//								console.log('Clearing ' + booking_count);
//								clearing = false;
//							}
							...
							$scope.patients = $scope.originalPatient ;
			...
			$scope.checkCurrentDate = function(m){
				...
				console.log(checkBookings(booking_count) + ' ' + booking_count + ' (' + calObj + ' - ' + testDate + ')');
				return the_result;
    ...
    $http.get('/api/main')
        .success(function(data) {
            ...
            console.log('api/main: ' + checkBookings(booking_count) + ' ' + booking_count);
...
//function mainController($scope, $http) {
 ...
 .controller('PatientController',function($scope, $http, $window) {
    ...
    $scope.getTime = function() {
		...
		$http.get('/api/bookings/' + $scope.formData.pt_appt_practice + '/' + sYear + '/' + sMonth + '/' + sDay)
            .success(function(data) {
...
*/ 
 ...
 .controller('LoginController',function($scope, $http, $location, AuthService) {
		...
		$scope.login = function() {
			...
			.then(function() {
				$scope.username = AuthService.getUserStatus();
				console.log('Status: ' + JSON.stringify($scope.username, null, 2));
  ...
  angular
    ...
    .factory('AuthService', ['$q','$timeout','$http','$rootScope', function($q, $timeout, $http, $rootScope) {
		...
		return({
			...
			function register(username, password, fname, sname) {
				var deferred = $q.defer();
				$http.post('/api/register',{username:username,password:password,fname:fname,sname:sname})
				.success(function(data,status) {
					if (status ===200 && data.status) {
						user = true;
						deferred.resolve();
