...
*/
	...
	app.get('/api/main/:sYear/:sMonth/:sDay', function(req, res, next) {
        ...
        Appt.aggregate([
				...
				var inter = JSON.stringify({"_declaration":{"_attributes":{"version":"1.0","encoding":"utf-8"}},Patients :{"Patient" : patients} } , null, 2);
				console.log(inter2);				
				var result = convert.json2xml(inter,options);		
				
				
				var expiry1 = new Date();
				expiry1.setDate(expiry1.getDate() + 7);
				...
				Request.post({
					...
					if(error) {
						...
						var body2 = JSON.stringify(body1 , null, 2)
						console.log(body2);
						
							Object.keys(body1.result).forEach(function(k){

								console.log(k + ' - ' + body1.result[k]._id);
								Appt.findByIdAndUpdate(body1.result[k]._id,{
										mfi_sent: 1
								}
								, function(err, appt) {
										if (err)
